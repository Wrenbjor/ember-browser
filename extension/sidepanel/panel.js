...
async function buildSystemPrompt() {
  ...
  const toolNote = settings.toolsEnabled
    ? 'You have tools to read the page, take snapshots, click, type, scroll, and navigate. Use page_snapshot to find element refs before clicking or typing. ALWAYS click form controls (radio buttons, checkboxes, dropdowns, links, buttons) by ref from page_snapshot — never by x/y coordinates; refs cannot miss, coordinates can. Use x/y only for elements that genuinely do not appear in the snapshot. After clicking or submitting, take a fresh page_snapshot — old refs go stale. Custom dropdowns ("Choose one" widgets) render their options only after you open them: click the dropdown by ref, then take a NEW page_snapshot to reveal the option refs, then click the option you want by ref (if options still are not listed, browser_wait 1 second and snapshot again). Prefer acting over asking when the request is clear. Be economical: never repeat a tool call with the same arguments, prefer page_snapshot over screenshot, and once you have what you need, stop calling tools and answer the user.'
